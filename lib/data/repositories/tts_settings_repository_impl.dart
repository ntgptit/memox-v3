...
class TtsSettingsRepositoryImpl implements TtsSettingsRepository {
  ...
  Future<Result<TtsSettings>> load() async {
    try {
      final TtsSettingsRow? row = await _dao.loadSettings();
  ...
  static TtsSettings _fromRow(TtsSettingsRow row) {
    final String? trimmedVoice = row.frontVoiceName?.trim();
    return TtsSettings(
      ...
      frontVoiceName:
          (trimmedVoice == null || trimmedVoice.isEmpty) ? null : trimmedVoice,
    );
  }

  static TtsSettingsCompanion _toCompanion(TtsSettings s) =>
      TtsSettingsCompanion(
        id: const Value<String>('default'),
        autoPlay: Value<bool>(s.autoPlay),
