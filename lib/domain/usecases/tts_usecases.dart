...
class UpdateTtsSettingsUseCase {
  ...
  Future<Result<void>> updateRate(double rate) => _modify(
    (TtsSettings s) => s.copyWith(rate: TtsSettings.normalizeRate(rate)),
  );

  Future<Result<void>> updatePitch(double pitch) => _modify(
    (TtsSettings s) => s.copyWith(pitch: TtsSettings.normalizePitch(pitch)),
  );

  Future<Result<void>> updateVolume(double volume) => _modify(
    (TtsSettings s) => s.copyWith(volume: TtsSettings.normalizeVolume(volume)),
  );
...
class SpeakFlashcardUseCase {
  ...
  Future<Result<void>> speakFlashcardFront({
    ...
    if (StringUtils.trimmed(frontText).isEmpty) {
      return const Result<void>.ok(null);
    }
  ...
  Future<Result<void>> speakText({
    ...
    if (StringUtils.trimmed(text).isEmpty) {
      return const Result<void>.ok(null);
    }
