# Claude Code Task Prompt — WBS 9.15: Unified verify entry point — ENFORCED

**Generated:** 2026-06-20
**Flow:** Quality | **Layer:** Test | **Status:** Specified

**Deliverable:**
> `node tool/verify/run.mjs`: one command, canonical chain, scope auto-detection, single summary. **Enforcement (2026-06-12, after piecemeal-verify relapse):** `--quick` legalizes the inner dev loop through the same entry; a docs/code PASS writes a content-state-bound pass-marker; the pre-commit hook (`--check-marker`) rejects commits without a matching marker and requires a code-chain marker when code is staged. Running verification commands individually is now a hard-rule violation AND technically uncommittable

## ⚠️ Dependency warnings

⚠️  Dependency `9.11` (doc_guard docs/process gate) is **Specified** — build it first.
...
**WBS ID:** `9.15`
**Evidence / Source:** `tool/verify/run.mjs`, `.githooks/pre-commit`, `CLAUDE.md` §Hard rules + §Verification; enforcement live-tested (commit blocked without marker → PASS → commit allowed)
...
```
| `<8-char-hash>` | 2026-06-20 | 9.15 | {one-line summary of what was implemented} |
```
