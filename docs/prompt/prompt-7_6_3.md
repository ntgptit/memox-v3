# Claude Code Task Prompt — WBS 7.6.3: Card history screen FE

**Generated:** 2026-06-20
**Flow:** Progress | **Layer:** FE | **Status:** Specified

**Deliverable:**
> `CardHistoryScreen` mock-parity (kit screen 09, all 5 states): breadcrumb + header (box chip) + CURRENT PROGRESS card (box stepper + 6-stat grid) + unified activity feed on a left rail (attempt rows with chip/desc/box→box·mode·duration; lifecycle rows; "All events" filter; "Beginning of history") + Edit pill + Reset/Delete overflow; duration measured in study review viewmodel; route `/library/deck/:deckId/flashcards/:flashcardId/history`

## ⚠️ Dependency warnings

⚠️  Dependency `7.6.2` (Card history use cases) is **Specified** — build it first.
...
**WBS ID:** `7.6.3`
**Evidence / Source:** `lib/presentation/features/history/**`, `lib/presentation/features/flashcards/widgets/flashcard_row_actions_sheet.dart`, `lib/app/router/route_paths.dart`, `lib/app/router/route_names.dart`, `lib/presentation/features/flashcards/routes/flashcard_routes.dart`, `test/presentation/features/history/card_history_screen_test.dart`
...
```
| `<8-char-hash>` | 2026-06-20 | 7.6.3 | {one-line summary of what was implemented} |
```
