...
| Study session             | `/library/study/session/:sessionId` — **built (WP-SR2..SR4b):** `StudySessionScreen` loads the review queue and renders the immersive ✕ shell + the both-sides **swipe-grade review surface** (right → `perfect`, left → `forgot`), the mid-session **exit-confirm** (WP-SR4a), and the long-press **card-actions** sheet (Bury / Suspend, WP-SR4b). Finish finalizes then `pushReplacement`s to the result route (WP-SR5a). **`?mode=match` (WP-SM3)** dispatches to the `MatchSessionScreen` board surface instead (blue MATCH pill + board grid; tap-pair interaction = WP-SM4); any other / absent `mode` → the default Review surface                                                                                                                                                          | No            |
...
- Study session route protects accidental exit and waits for explicit Finish Session before leaving the session screen. `?mode=match` dispatches to the Match board surface (`MatchSessionScreen`); any other or absent `mode` → the default Review swipe-grade surface (`StudySessionScreen`). The per-phase `current_mode` chain (WBS 4.5.12+) supersedes this query later.
