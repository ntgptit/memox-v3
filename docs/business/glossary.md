...
- `docs/database/schema-contract.md` — column names are aligned to glossary terms (e.g.,
  `entry_ref_id`, `box_before`, `last_reset_at`)

**Maintenance rule:**

- When renaming a term, run `grep -rn "{old_term}" docs/` and update ALL refs in the same commit,
  including this glossary. Per `CLAUDE.md` §Doc-code parity rule.
