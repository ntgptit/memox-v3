...
| TTS / audio settings| Partial — settings persistence Implemented (WBS 8.4.1, schema v9: `tts_settings` table + model/repo/usecases); the speech engine (`TtsService`/`FlutterTtsService`) + the Audio & speech screen (kit 23, 8.4.2) are pending.| `docs/business/tts/tts-settings.md`, `docs/contracts/repository-contracts/tts-settings-repository.md`|
