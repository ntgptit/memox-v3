...
| Streak + goal-progress READ surfaces| Engagement READ BE Implemented (Q5, WBS 7.4.3, no migration) — `LoadProgressEngagementUseCase` + `ProgressEngagement` compose the daily goal (`LearningSettings`) with attempt-derived study-day streak/today-count (`ProgressRepository.loadStudyActivity`, read-only). The kit-19 Progress detail (`GoalRing`/streak chip) consumes it; that **FE is pending**. Streak/reminder **persistence** (settings-backed `lastGoalMetDate`, `RecordGoalProgressUseCase`) + reminders stay Future (notification dep + permission).| `docs/business/engagement/dashboard-engagement.md`, `docs/contracts/usecase-contracts/engagement.md`, `docs/wireframes/03-progress.md`|
