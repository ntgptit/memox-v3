...
| 9 | 23-audio-speech | 🟡 in-progress | [#41](https://github.com/ntgptit/memox-v3/pull/41) (BE) | **Settings persistence BE shipped (WBS 8.4.1, PR #41, `4481ac9`):** `tts_settings` Drift table + **v9 schema migration** (the boot-critical part, landed reviewable on its own + migration test) + `TtsSettings` model/normalization + `TtsFrontLanguage` + DAO + repo + `Get`/`UpdateTtsSettingsUseCase` + DI (16 tests). **Remaining (8.4.2):** the `TtsService`/`FlutterTtsService` engine adapter (voice listing + speak/stop + auto-play gating; `flutter_tts` present, can't be verified headless → fake in tests) + the 7-state Audio & speech screen (Korean/English/Playing/Loading/No-voices/Engine-error/Saving). |
