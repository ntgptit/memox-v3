...
| C20 | Create/edit editor tags | User adds a valid tag (inline field, submit) | Validate + lowercase + dedupe (`TagValidator`), append the chip in the draft; the editor's full tag set replaces wholesale on save | C1 | `test/presentation/features/decks/flashcard_editor_test.dart` (add a tag → lowercased chip; comma rejected; save passes the full tag set) |
| C21 | Create/edit editor tags | User taps a chip's remove ✕ | Remove the chip from the draft (dirties the form → discard-confirm on close) | C1 | `test/presentation/features/decks/flashcard_editor_test.dart` (remove a tag drops the chip; removing dirties → discard confirm) |
...
| C43 | Card-row SRS subtitle | Flashcard-list `list-row-meta` (WP-FL1) | From `FlashcardListDetail.progressById[card.id]`: no entry or `dueAt == null` → `New · not studied`; else `days = dueAt.difference(now).inDays` (truncated), `days >= 1` → `Box {box} · due in {days}d`, else → `Box {box} · due today`. **Boundary:** SRS is day-granular, so anything **less than 24h** from due (or overdue) reads `due today` — a card due tomorrow but `< 24h` away intentionally collapses to `due today`. The mock's status **chip** is a mock visual gap (business model is New/Due only, `srs-review.md` §Rules) — not rendered | C0+C1 | `test/data/repositories/flashcard_repository_impl_test.dart` (read model carries progress), `test/presentation/features/decks/flashcard_tile_test.dart` (subtitle variants + goldens) |
| C44 | Editor Details expander | Card editor (WP-FL2b2) | A `Details · Optional` row toggles the optional **example / pronunciation / hint** fields (auto-opens in edit when the card has any non-empty); blank-after-trim is saved as `null`. Per **PRECEDENCE #1** (`flashcard-management.md` §V1) the mock's deck-selector is **Future** (deck retargeting) and its single "Note" maps to these business fields — built instead of the mock layout. The Details section also hosts the **TAGS** editor (WP-FL2b2b, C20/C21): the chip row + inline Add-tag field; the editor manages the card's full tag set, **replacing** tags wholesale on save (create + update). Details auto-opens in edit when the card has any non-empty field **or any tag** | C0+C1 | `test/presentation/features/decks/flashcard_editor_test.dart` (expander toggle + edit prefill + tag add/remove/save + details-open golden) |
