...
| C44 | Editor Details expander | Card editor (WP-FL2b2) | A `Details · Optional` row toggles the optional **example / pronunciation / hint** fields (auto-opens in edit when the card has any non-empty); blank-after-trim is saved as `null`. Per **PRECEDENCE #1** (`flashcard-management.md` §V1) the mock's deck-selector is **Future** (deck retargeting) and its single "Note" maps to these business fields — built instead of the mock layout. Edit **preserves** the card's existing tags on save (the update use case replaces tags wholesale; the tag editor = WP-FL2b2b) | C0+C1 | `test/presentation/features/decks/flashcard_editor_test.dart` (expander toggle + edit prefill + details-open golden) |
