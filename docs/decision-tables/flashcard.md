...
| C39 | Flashcard list filters | Tag composition / no-results | Tag filter composes with search (status filter Future — WBS 2.17.1); no-results keeps `totalCount` at the full deck total | C0+C1 | `test/data/repositories/flashcard_repository_impl_test.dart` (C39) |
