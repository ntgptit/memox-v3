...
| S94 | Study session mode dispatch (FE) | The session route builder receives `?mode=` (WP-SM3) | `mode == StudyMode.match.name` → render `MatchSessionScreen` (the Match board surface); any other or absent value → the default `StudySessionScreen` (Review). The per-phase `current_mode` chain (WBS 4.5.12+) supersedes this query later | C0 | `test/presentation/features/study/match_session_screen_test.dart` (board shell renders for the match surface) |
