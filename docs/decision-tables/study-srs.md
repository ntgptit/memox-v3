...
| S88 | Review session shell + card (FE) | Open `/library/study/session/:sessionId` (WP-SR2) | `StudySessionScreen` loads `studySessionReviewProvider` (`LoadStudySessionReviewUseCase`) and renders: `✕` exit + a blue `MxProgressBar` (`answeredCount/total`) + `{answered}/{total}` count; the current card (`items[firstUnansweredIndex ?? 0]`) showing **both sides at once** — front-side label (FRONT fallback; language label = WP-SR2b) → front → divider → back-side label (BACK) → back → **example pill** iff `exampleSentence` non-empty (no `note`/`pronunciation`/`hint`, no reveal step); loading / load-error (Retry) / empty states. A fully-answered reload (`firstUnansweredIndex == null`) falls back to the first card with a full progress bar; the Finish-Session surface for that case is WP-SR3. Swipe-grade = WP-SR3 | C0+C1 | `test/presentation/features/study/study_session_screen_test.dart` (loaded/example/progress/loading/error/empty + card goldens) |
