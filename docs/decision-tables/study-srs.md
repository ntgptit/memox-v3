...
| S46 | Study mode strategy | `StudyMode.review` resolved | Return `ReviewStudyModeStrategy` with `perfect` / `forgot` grading and no reveal/self-grade flow | C1 | TBD |
| S47 | Study mode strategy | `StudyMode.guess` resolved | Return `GuessStudyModeStrategy` with deterministic option builder and `perfect` / `forgot` grading | C1 | TBD |
| S48 | Study mode strategy | `StudyMode.match` resolved | Return `MatchStudyModeStrategy` (Board family: exposes no per-card grading API); Match uses append-only evaluation persistence, derives terminal attempts at finalization, and never emits `initial_passed` | C1 | TBD |
| S49 | Study mode strategy | `StudyMode.fill` resolved | Return `FillStudyModeStrategy` (TypedAnswer family: evaluator-graded, no Forgot/Got-it API) with strict trim-only matching, one terminal persisted attempt, and no reveal/self-grade flow | C1 | TBD |
...
| S58 | Study mode strategy | Any `StudyMode` value resolved | Every resolved strategy belongs to exactly one sealed interaction family (`BinaryGradeStudyModeStrategy` / `TypedAnswerStudyModeStrategy` / `BoardStudyModeStrategy`) and reports the resolved mode | C1 | TBD |
