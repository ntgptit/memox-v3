...
| S84 | Study entry gate (FE) | Gate resolves the scope's start outcome (WP-SR1a) | `StudyEntryScreen` renders: preparing while resolving; an empty surface for `blocked` (generic copy, WP-SR1a — per-reason matrix = WP-SR1b-2) without creating a session; Resume / Start over / Back for `resumeRequired` (no silent resume, S28); auto-creates a session for an eligible scope and `pushReplacement`s to it (`ready`); an error surface on failure or an unparseable `entryType` | C0+C1 | `test/presentation/features/study/study_entry_screen_test.dart` |
