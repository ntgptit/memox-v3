...
| S86 | Study entry empty matrix (FE) | `blocked(reason)` (WP-SR1b-2a) | `_blockedBody` switches the 8 `StudyScopeEmptyReason` → a tailored icon + title + message (`studyEmpty*` ARB; cards/check/celebration/bedtime/pause glyphs) instead of one generic surface; always with a Back action, always blocking the session. The dedicated CTAs (Add flashcards / Study new instead / View suspended), the "Next due in {relativeTime}" line, and the streak inset are **WP-SR1b-2b** | C0+C1 | `test/presentation/features/study/study_entry_screen_test.dart` (per-reason title + empty goldens) |
