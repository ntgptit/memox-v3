...
| F12 | Deck counts | Folder with direct decks + cards | `deckCount` = direct child decks; `cardCount` includes all cards (incl. NEW, suspended, buried); `dueCount` = scheduled cards with `due_at <= now` (NEW excluded), **excluding suspended (`COALESCE(is_suspended,0)=1`) and currently-buried (`buried_until > now`) cards; an expired bury (`buried_until <= now`) counts as due** — implemented in `folder_queries.drift`, mirrors `study_scope_queries.drift` | C0+C1 | `folder_deck_due_counts_test.dart` |
| F13 | Recursive counts | Cards in descendant folders' decks | Recursive subtree counts: `cardCount` includes all subtree cards (incl. suspended/buried); `dueCount` = subtree cards with `due_at <= now` (NEW excluded), **excluding suspended and currently-buried cards; expired bury counts as due** — implemented in `folder_queries.drift` | C0+C1 | `folder_deck_due_counts_test.dart` |
...
| F23 | New-card count | Folder-summary `newCount` (mock `03a` badge) | `newCount` = active cards never yet studied in the subtree (`due_at IS NULL`), **excluding** suspended (`COALESCE(is_suspended,0)=1`) and currently-buried (`buried_until > now`) cards. Stricter than the study new-queue (WBS 4.1.1 draws New from all active cards; new-vs-due split is 3.2.3, Future) — the badge surfaces unseen cards | C0+C1 | `folder_deck_due_counts_test.dart` |
| F24 | Mastery | Folder-summary `mastery` (mock `03a` progress bar) | `mastery` = `AVG(COALESCE(box_number,1)) / SrsBox.max` over **all** subtree cards (includes suspended/buried — mastery is over the full corpus, not the active queue); `null` when the subtree has no cards | C0+C1 | `folder_deck_due_counts_test.dart` |
