...
| P16 | Compute streak | Consecutive study days exist; today has no attempt yet | Current streak counts back from yesterday (an unfinished today does not break it); longest streak scans whole history | C1 | `progress_repository_engagement_test.dart` |
...
| P18 | Load overview | Empty database, any range | Return zero-safe overview (zero totals, zero streak, zero card states, full zero-filled day buckets for week/month) | C1 | `progress_repository_engagement_test.dart` (engagement slice: zero today/streak) |
