...
| Breadcrumb                      | **Current (design redesign).** Shared `MxBreadcrumb` trail docked directly under the app bar (built via `buildLibraryBreadcrumb` in `lib/presentation/shared/widgets/navigation/library_breadcrumb.dart`): `Library › …ancestors › {currentFolder}`. Chevron (`›`) separators; scrolls horizontally on deep paths; the last crumb (current folder) is bold + non-tappable; `Library` taps to the branch root (`goNamed`), each ancestor folder pushes its detail. Hidden in search mode and until the folder has loaded. |
