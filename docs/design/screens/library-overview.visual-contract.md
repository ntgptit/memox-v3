...
1. Business and route scope: `docs/business/folder/folder-management.md`, `docs/business/deck/deck-management.md`, `docs/business/search/global-search.md`, `docs/business/navigation/navigation-flow.md`.
...
4. Visual reference: `docs/system-design/MemoX Design System/ui_kits/mobile/index.html` - `03 · Library overview` (`03a`-`03f`).
5. Design tokens and shared components: `docs/design/design-token-mapping.md`, `docs/design/component-visual-contract.md`, `docs/ui-ux/ui-ux-contract.md`.
...
| Folder kebab affordance | Current visual | Visible trailing `Icons.more_vert`; currently disabled if no action handler exists. |
| Folder action sheet | Future / deferred | Current code passes `onShowActions: null`; do not document as implemented until action sheet code and tests exist. |
| Sort pill `Recent` | Future | Sort exists in data/use-case layer, but no sort UI control is exposed. |
...
| `03f` | Overflow visual | Mock shows folder sheet | Visible kebab remains; sheet is not Current until implemented | No action sheet unless handler exists | Folder mutation actions | Treat sheet actions as Future/deferred in V1 docs. |
...
4. Trailing action.
   ...
   - Disabled when no approved action handler exists.
...
- Do not wire the mock overflow sheet until folder action-sheet behavior, shared component usage, l10n, tests, and docs are implemented.
...
- Header, inline search, loaded list, FAB, and bottom nav visually resemble the approved mock within current design-system tokens.
