...
> screen `04`), not bare full-height–centered content. See
> `library_overview_body._panelInCard`. Inner-panel refinements still pending as a shared
...
| `03c`   | Empty          | `totalFolderCount == 0`                            | `MxEmptyState` panel inside the grouped content card (top-anchored), with create-folder CTA                                              | Create folder → `showMxFolderCreateDialog`                                                       | `totalFolderCount`                                               | **Current.** Card-wrapped via `_panelInCard` (rev. 4). No root New deck / Import CTA (Rejected). Distinct from search no-results (see Derived states). |
| `03d`   | Error          | Query failure                                      | `MxErrorState` panel inside the grouped content card (top-anchored), localized title/message + Retry                                     | Retry → `ref.invalidate(libraryOverviewQueryProvider)`                                           | Async error / failure                                            | **Current.** Card-wrapped via `_panelInCard` (rev. 4). No raw exception text surfaced.                                                            |
