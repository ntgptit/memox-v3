...
| `03a`   | Loaded            | Query returns folders                                       | Header, inline search, optional keycap, optional due summary, folder count header, folder cards, FAB, bottom nav | Tap folder navigates to folder detail; create folder opens dialog | `LibraryOverviewReadModel.folders`, `dueToday`, recursive counts | Search shortcut badge only appears while the field is empty.      |
