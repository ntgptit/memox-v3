...
> **Rev. 4 (2026-06-23, empty/error card parity):** the **Empty** (`03c`) and **Error**
> (`03d`) states now render their `MxEmptyState`/`MxErrorState` panel **inside the grouped
> content card** (`MxCard` with padding zeroed so the panel's own 24px inset is the card
> pad), anchored at the top of the scroll body — matching the kit Empty/Error shots (and
> screen `04`), not bare full-height–centered content. The card-wrap composition lives in the
> shared `MxStateCard` (`lib/presentation/shared/widgets/states/mx_state_card.dart`), reused by
> Folder detail (rev. 4 → shared 2026-06-23). Inner-panel refinements still pending as a shared
> `MxEmptyState`/`MxErrorState` follow-up: 56px tile (needs a size token), solid-accent
> empty tile vs tonal error tile, and the 22/800 tracking -0.4 title.
...
| `03c`   | Empty          | `totalFolderCount == 0`                            | `MxEmptyState` panel inside the grouped content card (top-anchored), with create-folder CTA                                              | Create folder → `showMxFolderCreateDialog`                                                       | `totalFolderCount`                                               | **Current.** Card-wrapped via shared `MxStateCard` (rev. 4). No root New deck / Import CTA (Rejected). Distinct from search no-results (see Derived states). |
| `03d`   | Error          | Query failure                                      | `MxErrorState` panel inside the grouped content card (top-anchored), localized title/message + Retry                                     | Retry → `ref.invalidate(libraryOverviewQueryProvider)`                                           | Async error / failure                                            | **Current.** Card-wrapped via shared `MxStateCard` (rev. 4). No raw exception text surfaced.                                                      |
