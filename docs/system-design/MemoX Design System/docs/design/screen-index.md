...
| Study entry gate | `/library/study/:entryType/:entryRefId`, `/library/study/today` | (gate; see `12 · Study` group) | `wireframes/12-study-entry-gate.md`; `business/study/study-flow.md` | `features/study/screens/study_entry_screen.dart`, `features/study/providers/study_entry_notifier.dart`, `features/study/routes/study_routes.dart` | _TBD_ | **Partial** | P3 | Real route gate with preparing/error/unsupported-gap states. Session creation/resume remains Future and the session/result routes are still placeholders. |
