# 05 — Library search — DOM spec

Auto-generated by `tool/ui_kit_shots/export_specs.mjs` from the rendered UI kit. Do not
edit by hand; re-run the exporter after any `../index.html` change (the freshness check
in `tool/verify/run.mjs` fails when this is stale).

Reading guide: each visible node is a block; one short head line, then grouped field lines.
`- [item[i]] name "own text" mx:<Mx> abs:[x,y WxH] rel:[x,y WxH] <layout> <flex-child> repeat:xN(unit=P) pad:t/r/b/l margin:t/r/b/l minw/maxw/minh/maxh pos:... layout_hint:... z:N scrollh:N transform:... bg:<color> font:<size/weight[/line-height]> color:<color> text:<align> tracking:N r:<radius> border:<w>px <color> (or border-t/r/b/l for a single-side divider) shadow:<offY>/<blur>`.
Indentation = DOM containment (layout/grouping containers are kept, not flattened).
`abs:[...]` is frame-relative (cross-check with the PNG); `rel:[...]` is the box offset+size
INSIDE its parent - read spacing from rel, not abs, so the layout stays relative.
`<layout>` on a container is its child arrangement: `flex:row|col gap:N justify:... align:...`
or `grid cols:N` - map to a Flutter Row/Column/Wrap/GridView, not absolute coords.
`<flex-child>` is a flex item constraint: `grow:N shrink:N basis:N self:...` plus
`layout_hint:expanded` (-> Expanded) / `layout_hint:flexible` (-> Flexible).
`pad`/`margin` are 4-edge (collapsed: `N` all-equal, `V/H`, or `t/r/b/l`); `minw/maxw/minh/maxh`
are explicit size constraints. `pos:` is non-static positioning; `layout_hint:scroll` =
scroll container, `layout_hint:pinned` = sticky/fixed (bottom bars, sheets, FABs), `clip` =
overflow hidden, `z:N` = stacking - use these to decide Stack/Positioned/bottomSheet vs flow.
`repeat:xN(unit=P)` marks a list of N items of P elements each; `item[i]` tags each unit
start - build it as a list/builder, not N copies (a +N suffix means a trailing partial unit).
`scrollh:N` is the scroll content height (vs the viewport `WxH`); `transform:...`,
`tracking:N` (letter-spacing px), `text:<align>` are emitted only when set.
`shadow:<offY>/<blur>` is the box-shadow -> map to an elevation. Coordinates are px on the
390x780 phone frame (light theme measured; dark remaps the same `--memox-*` tokens). A
`<color>` is a `--memox-*` token name; `token@NN` / `#rrggbb@NN` = that color at NN% opacity
(overlay/tint, not a hardcoded color). Token names map to Flutter symbols via
`docs/design/design-token-mapping.md`; a bare `#rrggbb` means no token matched - treat as a
gap, not a license to hardcode. Non-base states are an ordered diff (`+` added / `-` removed
in document order with abs+rel bbox kept, `...` = unchanged run). Every quoted "..." string is
MOCK COPY - the kit carries NO l10n keys; never copy it into the app, source real strings from
ARB (`docs/design/mock-design-index.md`). Numbers/counts are illustrative, not the system
contract. `mx:<Mx>` is the suggested MemoX shared component (grounded in
`docs/design/component-visual-contract.md`); `mx:?` is an interactive control with no
confident mapping (resolve via that contract). When no `mx:` is present, `name` is just the
raw kit CSS class (e.g. `ov`, `title`) and is NOT a resolved component. Two mappings stay
deliberately MISSING, not guessed: a bare `#rrggbb` is an un-tokenized color, and quoted text
has no l10n key. Visual reference PNGs: `../shots/` (see `../shots/INDEX.md`).

## Base state: Empty

```text
- node: app
  mx: MxScaffold
  box:
    abs: [8,8 390x780]
    rel: [8,8 390x780]
  layout: flex:col
  position: pos:relative clip
  style: bg:surface
  - node: statusbar
    box:
      abs: [8,8 390x44]
      rel: [0,0 390x44]
    layout: flex:row justify:between align:center
    spacing: pad:0/24
    - node: span
      text: 9:41
      box:
        abs: [32,21 28x18]
        rel: [24,13 28x18]
      style: font:14/600 color:font-headline
    - node: span
      box:
        abs: [314,24 60x12]
        rel: [306,16 60x12]
      layout: flex:row gap:4 align:center
      - node: svg
        box:
          abs: [314,24 16x12]
          rel: [0,0 16x12]
        position: clip
      - node: svg
        box:
          abs: [334,24 14x12]
          rel: [20,0 14x12]
        position: clip
      - node: svg
        box:
          abs: [352,24 22x12]
          rel: [38,0 22x12]
        position: clip
  - node: appbar
    mx: MxAppBar
    box:
      abs: [8,52 390x56]
      rel: [0,44 390x56]
    layout: flex:row gap:8 align:center
    spacing: pad:0/8
    - node: icon-btn
      mx: MxIconButton
      box:
        abs: [16,62 36x36]
        rel: [8,10 36x36]
      layout: flex:row justify:center align:center
      position: pos:relative
      style: r:999
      - node: span
        box:
          abs: [24,70 20x20]
          rel: [8,8 20x20]
        layout: flex:row
        - node: icon:arrow-left
          box:
            abs: [24,70 20x20]
            rel: [0,0 20x20]
          position: clip
    - node: div
      box:
        abs: [60,60 330x40]
        rel: [52,8 330x40]
      layout: flex:row gap:8 align:center
      flex: grow:1 basis:0 layout_hint:expanded
      spacing: pad:0/12
      style: bg:on-primary r:12 border:1px seed-indigo
      - node: span
        box:
          abs: [73,73 15x15]
          rel: [13,13 15x15]
        layout: flex:row
        - node: icon:search
          box:
            abs: [73,73 15x15]
            rel: [0,0 15x15]
          position: clip
      - node: span
        text: Search library
        box:
          abs: [96,71 281x18]
          rel: [36,11 281x18]
        layout: flex:row align:center
        flex: grow:1 basis:0 layout_hint:expanded
        position: clip
        style: font:14/500 color:on-surface-variant
        - node: span
          box:
            abs: [191,72 2x16]
            rel: [95,1 2x16]
          spacing: margin:0/0/0/4
          style: bg:seed-indigo
  - node: scroll
    box:
      abs: [8,108 390x680]
      rel: [0,100 390x680]
    flex: grow:1 basis:0 layout_hint:expanded
    repeat: x2+(unit=2)
    spacing: pad:0/14/14/14
    position: layout_hint:scroll
    - node: div
      item: item[1]
      box:
        abs: [22,108 362x25]
        rel: [14,0 362x25]
      layout: flex:row justify:between align:center
      spacing: pad:2/4/8/4
      - node: div
        box:
          abs: [26,111 125x13]
          rel: [4,3 125x13]
        layout: flex:row gap:6 align:center
        - node: span
          box:
            abs: [26,112 11x11]
            rel: [0,1 11x11]
          layout: flex:row
          - node: icon:clock
            box:
              abs: [26,112 11x11]
              rel: [0,0 11x11]
            position: clip
        - node: ov
          text: Recent searches
          box:
            abs: [43,111 108x13]
            rel: [17,0 108x13]
          style: font:10.5/600 color:on-surface-variant tracking:0.6 op:0.85
      - node: button
        text: Clear
        mx: ?
        box:
          abs: [349,110 31x15]
          rel: [327,2 31x15]
        style: font:12/600 color:on-surface-variant text:center
    - node: card
      mx: MxCard
      box:
        abs: [22,133 362x223]
        rel: [14,25 362x223]
      repeat: x4(unit=1)
      spacing: margin:0/0/18/0
      position: clip
      style: bg:on-primary r:20 shadow:1/2
      - node: div
        item: item[1]
        box:
          abs: [22,133 362x57]
          rel: [0,0 362x57]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:10/14
        style: border-b:1px seed-indigo@14
        - node: span
          box:
            abs: [36,155 24x13]
            rel: [14,22 24x13]
          layout: flex:row
          - node: icon:rotate-ccw
            box:
              abs: [36,155 13x13]
              rel: [0,0 13x13]
            position: clip
        - node: div
          box:
            abs: [72,143 262x36]
            rel: [50,10 262x36]
          - node: div
            text: 연구자
            box:
              abs: [72,143 262x20]
              rel: [0,0 262x20]
            position: clip
            style: font:14/600 color:font-headline
          - node: div
            text: 12 results
            box:
              abs: [72,164 262x15]
              rel: [0,21 262x15]
            spacing: margin:1/0/0/0
            style: font:12/400 color:on-surface-variant
        - node: icon-btn
          mx: MxIconButton
          box:
            abs: [346,149 24x24]
            rel: [324,16 24x24]
          layout: flex:row justify:center align:center
          position: pos:relative
          style: r:999
          - node: span
            box:
              abs: [348,151 20x20]
              rel: [2,2 20x20]
            layout: flex:row
            - node: icon:x
              box:
                abs: [348,151 20x20]
                rel: [0,0 20x20]
              position: clip
      - node: div
        item: item[2]
        box:
          abs: [22,190 362x55]
          rel: [0,57 362x55]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:10/14
        style: border-b:1px seed-indigo@14
        - node: span
          box:
            abs: [36,211 24x13]
            rel: [14,21 24x13]
          layout: flex:row
          - node: icon:rotate-ccw
            box:
              abs: [36,211 13x13]
              rel: [0,0 13x13]
            position: clip
        - node: div
          box:
            abs: [72,200 262x34]
            rel: [50,10 262x34]
          - node: div
            text: TOPIK II
            box:
              abs: [72,200 262x18]
              rel: [0,0 262x18]
            position: clip
            style: font:14/600 color:font-headline
          - node: div
            text: 7 results · folder
            box:
              abs: [72,219 262x15]
              rel: [0,19 262x15]
            spacing: margin:1/0/0/0
            style: font:12/400 color:on-surface-variant
        - node: icon-btn
          mx: MxIconButton
          box:
            abs: [346,205 24x24]
            rel: [324,15 24x24]
          layout: flex:row justify:center align:center
          position: pos:relative
          style: r:999
          - node: span
            box:
              abs: [348,207 20x20]
              rel: [2,2 20x20]
            layout: flex:row
            - node: icon:x
              box:
                abs: [348,207 20x20]
                rel: [0,0 20x20]
              position: clip
      - node: div
        item: item[3]
        box:
          abs: [22,245 362x55]
          rel: [0,112 362x55]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:10/14
        style: border-b:1px seed-indigo@14
        - node: span
          box:
            abs: [36,266 24x13]
            rel: [14,21 24x13]
          layout: flex:row
          - node: icon:rotate-ccw
            box:
              abs: [36,266 13x13]
              rel: [0,0 13x13]
            position: clip
        - node: div
          box:
            abs: [72,255 262x34]
            rel: [50,10 262x34]
          - node: div
            text: verb
            box:
              abs: [72,255 262x18]
              rel: [0,0 262x18]
            position: clip
            style: font:14/600 color:font-headline
          - node: div
            text: 1 tag · 71 cards
            box:
              abs: [72,274 262x15]
              rel: [0,19 262x15]
            spacing: margin:1/0/0/0
            style: font:12/400 color:on-surface-variant
        - node: icon-btn
          mx: MxIconButton
          box:
            abs: [346,260 24x24]
            rel: [324,15 24x24]
          layout: flex:row justify:center align:center
          position: pos:relative
          style: r:999
          - node: span
            box:
              abs: [348,262 20x20]
              rel: [2,2 20x20]
            layout: flex:row
            - node: icon:x
              box:
                abs: [348,262 20x20]
                rel: [0,0 20x20]
              position: clip
      - node: div
        item: item[4]
        box:
          abs: [22,300 362x56]
          rel: [0,167 362x56]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:10/14
        - node: span
          box:
            abs: [36,322 24x13]
            rel: [14,22 24x13]
          layout: flex:row
          - node: icon:rotate-ccw
            box:
              abs: [36,322 13x13]
              rel: [0,0 13x13]
            position: clip
        - node: div
          box:
            abs: [72,310 262x36]
            rel: [50,10 262x36]
          - node: div
            text: 도서관
            box:
              abs: [72,310 262x20]
              rel: [0,0 262x20]
            position: clip
            style: font:14/600 color:font-headline
          - node: div
            text: 1 card
            box:
              abs: [72,331 262x15]
              rel: [0,21 262x15]
            spacing: margin:1/0/0/0
            style: font:12/400 color:on-surface-variant
        - node: icon-btn
          mx: MxIconButton
          box:
            abs: [346,316 24x24]
            rel: [324,16 24x24]
          layout: flex:row justify:center align:center
          position: pos:relative
          style: r:999
          - node: span
            box:
              abs: [348,318 20x20]
              rel: [2,2 20x20]
            layout: flex:row
            - node: icon:x
              box:
                abs: [348,318 20x20]
                rel: [0,0 20x20]
              position: clip
    - node: ov
      text: Try searching
      box:
        abs: [26,381 108x13]
        rel: [18,273 108x13]
      layout: flex:row gap:6 align:center
      style: font:10.5/600 color:on-surface-variant tracking:0.6 op:0.85
      - node: span
        box:
          abs: [26,382 11x11]
          rel: [0,1 11x11]
        layout: flex:row
        - node: icon:sparkles
          box:
            abs: [26,382 11x11]
            rel: [0,0 11x11]
          position: clip
    - node: card
      mx: MxCard
      box:
        abs: [22,405 362x229]
        rel: [14,297 362x229]
      repeat: x4(unit=1)
      spacing: margin:0/0/14/0
      position: clip
      style: bg:on-primary r:20 shadow:1/2
      - node: div
        item: item[1]
        box:
          abs: [22,405 362x57]
          rel: [0,0 362x57]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:11/14
        style: border-b:1px seed-indigo@14
        - node: div
          box:
            abs: [36,420 26x26]
            rel: [14,15 26x26]
          layout: flex:row justify:center align:center
          style: bg:seed-indigo@12 r:8
          - node: span
            box:
              abs: [43,427 13x13]
              rel: [7,7 13x13]
            layout: flex:row
            - node: icon:folder
              box:
                abs: [43,427 13x13]
                rel: [0,0 13x13]
              position: clip
        - node: div
          box:
            abs: [78,416 266x34]
            rel: [56,11 266x34]
          - node: div
            text: a folder name
            box:
              abs: [78,416 266x18]
              rel: [0,0 266x18]
            position: clip
            style: font:14/600 color:font-headline tracking:-0.1
          - node: div
            text: Korean, Japanese
            box:
              abs: [78,435 266x15]
              rel: [0,19 266x15]
            spacing: margin:1/0/0/0
            position: clip
            style: font:12/400 color:on-surface-variant
        - node: div
          box:
            abs: [356,426 14x14]
            rel: [334,21 14x14]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [356,426 14x14]
              rel: [0,0 14x14]
            layout: flex:row
            - node: icon:arrow-up-left
              box:
                abs: [356,426 14x14]
                rel: [0,0 14x14]
              position: clip
      - node: div
        item: item[2]
        box:
          abs: [22,462 362x57]
          rel: [0,57 362x57]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:11/14
        style: border-b:1px seed-indigo@14
        - node: div
          box:
            abs: [36,477 26x26]
            rel: [14,15 26x26]
          layout: flex:row justify:center align:center
          style: bg:accent@12 r:8
          - node: span
            box:
              abs: [43,484 13x13]
              rel: [7,7 13x13]
            layout: flex:row
            - node: icon:layers
              box:
                abs: [43,484 13x13]
                rel: [0,0 13x13]
              position: clip
        - node: div
          box:
            abs: [78,473 266x34]
            rel: [56,11 266x34]
          - node: div
            text: a deck
            box:
              abs: [78,473 266x18]
              rel: [0,0 266x18]
            position: clip
            style: font:14/600 color:font-headline tracking:-0.1
          - node: div
            text: TOPIK II — Vocab
            box:
              abs: [78,492 266x15]
              rel: [0,19 266x15]
            spacing: margin:1/0/0/0
            position: clip
            style: font:12/400 color:on-surface-variant
        - node: div
          box:
            abs: [356,483 14x14]
            rel: [334,21 14x14]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [356,483 14x14]
              rel: [0,0 14x14]
            layout: flex:row
            - node: icon:arrow-up-left
              box:
                abs: [356,483 14x14]
                rel: [0,0 14x14]
              position: clip
      - node: div
        item: item[3]
        box:
          abs: [22,519 362x59]
          rel: [0,114 362x59]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:11/14
        style: border-b:1px seed-indigo@14
        - node: div
          box:
            abs: [36,535 26x26]
            rel: [14,16 26x26]
          layout: flex:row justify:center align:center
          style: r:8
          - node: span
            box:
              abs: [43,542 13x13]
              rel: [7,7 13x13]
            layout: flex:row
            - node: icon:copy
              box:
                abs: [43,542 13x13]
                rel: [0,0 13x13]
              position: clip
        - node: div
          box:
            abs: [78,530 266x36]
            rel: [56,11 266x36]
          - node: div
            text: a card term or meaning
            box:
              abs: [78,530 266x18]
              rel: [0,0 266x18]
            position: clip
            style: font:14/600 color:font-headline tracking:-0.1
          - node: div
            text: 연구자, library
            box:
              abs: [78,549 266x17]
              rel: [0,19 266x17]
            spacing: margin:1/0/0/0
            position: clip
            style: font:12/400 color:on-surface-variant
        - node: div
          box:
            abs: [356,541 14x14]
            rel: [334,22 14x14]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [356,541 14x14]
              rel: [0,0 14x14]
            layout: flex:row
            - node: icon:arrow-up-left
              box:
                abs: [356,541 14x14]
                rel: [0,0 14x14]
              position: clip
      - node: div
        item: item[4]
        box:
          abs: [22,578 362x56]
          rel: [0,173 362x56]
        layout: grid cols:3 gap:12 align:center
        spacing: pad:11/14
        - node: div
          box:
            abs: [36,593 26x26]
            rel: [14,15 26x26]
          layout: flex:row justify:center align:center
          style: r:8
          - node: span
            box:
              abs: [43,600 13x13]
              rel: [7,7 13x13]
            layout: flex:row
            - node: icon:tag
              box:
                abs: [43,600 13x13]
                rel: [0,0 13x13]
              position: clip
        - node: div
          box:
            abs: [78,589 266x34]
            rel: [56,11 266x34]
          - node: div
            text: a tag
            box:
              abs: [78,589 266x18]
              rel: [0,0 266x18]
            position: clip
            style: font:14/600 color:font-headline tracking:-0.1
          - node: div
            text: noun, verb, food
            box:
              abs: [78,608 266x15]
              rel: [0,19 266x15]
            spacing: margin:1/0/0/0
            position: clip
            style: font:12/400 color:on-surface-variant
        - node: div
          box:
            abs: [356,599 14x14]
            rel: [334,21 14x14]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [356,599 14x14]
              rel: [0,0 14x14]
            layout: flex:row
            - node: icon:arrow-up-left
              box:
                abs: [356,599 14x14]
                rel: [0,0 14x14]
              position: clip
    - node: div
      box:
        abs: [22,648 362x58]
        rel: [14,540 362x58]
      layout: flex:row gap:8 align:start
      spacing: pad:10/12
      style: bg:on-primary r:12 border:1px seed-indigo@14
      - node: span
        box:
          abs: [35,659 13x13]
          rel: [13,11 13x13]
        layout: flex:row
        - node: icon:info
          box:
            abs: [35,659 13x13]
            rel: [0,0 13x13]
          position: clip
      - node: span
        text: Search is case-insensitive. Korean, English, and Vietnamese all work.
        box:
          abs: [56,659 315x36]
          rel: [34,11 315x36]
        style: font:12/400/18 color:on-surface-variant
```

## State: Loading (full — differs too much from base)

```text
- node: app
  mx: MxScaffold
  box:
    abs: [8,8 390x780]
    rel: [8,8 390x780]
  layout: flex:col
  position: pos:relative clip
  style: bg:surface
  - node: statusbar
    box:
      abs: [8,8 390x44]
      rel: [0,0 390x44]
    layout: flex:row justify:between align:center
    spacing: pad:0/24
    - node: span
      text: 9:41
      box:
        abs: [32,21 28x18]
        rel: [24,13 28x18]
      style: font:14/600 color:font-headline
    - node: span
      box:
        abs: [314,24 60x12]
        rel: [306,16 60x12]
      layout: flex:row gap:4 align:center
      - node: svg
        box:
          abs: [314,24 16x12]
          rel: [0,0 16x12]
        position: clip
      - node: svg
        box:
          abs: [334,24 14x12]
          rel: [20,0 14x12]
        position: clip
      - node: svg
        box:
          abs: [352,24 22x12]
          rel: [38,0 22x12]
        position: clip
  - node: appbar
    mx: MxAppBar
    box:
      abs: [8,52 390x56]
      rel: [0,44 390x56]
    layout: flex:row gap:8 align:center
    spacing: pad:0/8
    - node: icon-btn
      mx: MxIconButton
      box:
        abs: [16,62 36x36]
        rel: [8,10 36x36]
      layout: flex:row justify:center align:center
      position: pos:relative
      style: r:999
      - node: span
        box:
          abs: [24,70 20x20]
          rel: [8,8 20x20]
        layout: flex:row
        - node: icon:arrow-left
          box:
            abs: [24,70 20x20]
            rel: [0,0 20x20]
          position: clip
    - node: div
      box:
        abs: [60,60 330x40]
        rel: [52,8 330x40]
      layout: flex:row gap:8 align:center
      flex: grow:1 basis:0 layout_hint:expanded
      spacing: pad:0/12
      style: bg:on-primary r:12 border:1px seed-indigo
      - node: span
        box:
          abs: [73,73 15x15]
          rel: [13,13 15x15]
        layout: flex:row
        - node: icon:search
          box:
            abs: [73,73 15x15]
            rel: [0,0 15x15]
          position: clip
      - node: span
        text: 연구
        box:
          abs: [96,70 249x20]
          rel: [36,10 249x20]
        layout: flex:row align:center
        flex: grow:1 basis:0 layout_hint:expanded
        position: clip
        style: font:14/600 color:font-headline
        - node: span
          box:
            abs: [124,72 2x16]
            rel: [28,2 2x16]
          spacing: margin:0/0/0/2
          style: bg:seed-indigo
      - node: icon-btn
        mx: MxIconButton
        box:
          abs: [353,68 24x24]
          rel: [293,8 24x24]
        layout: flex:row justify:center align:center
        position: pos:relative
        style: r:999
        - node: span
          box:
            abs: [355,70 20x20]
            rel: [2,2 20x20]
          layout: flex:row
          - node: icon:x-circle
            box:
              abs: [355,70 20x20]
              rel: [0,0 20x20]
            position: clip
  - node: scroll-x
    box:
      abs: [22,108 362x28]
      rel: [14,100 362x28]
    layout: flex:row gap:6
    repeat: x5(unit=1)
    position: layout_hint:scroll
    - node: button
      item: item[1]
      text: All
      mx: ?
      box:
        abs: [22,108 54x28]
        rel: [0,0 54x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:seed-indigo font:12/600 color:on-primary text:center r:999
      - node: span
        text: 18
        box:
          abs: [51,115 14x15]
          rel: [29,7 14x15]
        style: font:12/700 color:on-primary text:center op:0.75
    - node: button
      item: item[2]
      text: Folders
      mx: ?
      box:
        abs: [82,108 94x28]
        rel: [60,0 94x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [93,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:folder
          box:
            abs: [93,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 1
        box:
          abs: [157,115 7x15]
          rel: [76,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[3]
      text: Decks
      mx: ?
      box:
        abs: [181,108 87x28]
        rel: [159,0 87x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [192,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:layers
          box:
            abs: [192,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 2
        box:
          abs: [250,115 7x15]
          rel: [68,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[4]
      text: Flashcards
      mx: ?
      box:
        abs: [274,108 120x28]
        rel: [252,0 120x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [285,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:copy
          box:
            abs: [285,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 12
        box:
          abs: [369,115 14x15]
          rel: [95,7 14x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[5]
      text: Tags
      mx: ?
      box:
        abs: [400,108 77x28]
        rel: [378,0 77x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [411,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:tag
          box:
            abs: [411,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 3
        box:
          abs: [459,115 7x15]
          rel: [59,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
  - node: scroll
    box:
      abs: [8,146 390x642]
      rel: [0,138 390x642]
    flex: grow:1 basis:0 layout_hint:expanded
    spacing: pad:0/14/14/14
    position: layout_hint:scroll
    - node: ov
      text: Searching for "연구"…
      box:
        abs: [26,156 136x13]
        rel: [18,10 136x13]
      style: font:10.5/600 color:on-surface-variant tracking:0.6 op:0.85
    - node: div
      box:
        abs: [22,179 362x171]
        rel: [14,33 362x171]
      spacing: margin:0/0/14/0
      - node: div
        box:
          abs: [22,179 362x19]
          rel: [0,0 362x19]
        layout: flex:row gap:6
        spacing: pad:2/4/8/4
        - node: span
          box:
            abs: [26,181 70x9]
            rel: [4,2 70x9]
          style: bg:surface-container-high r:6 op:0.4
      - node: card
        mx: MxCard
        box:
          abs: [22,198 362x152]
          rel: [0,19 362x152]
        position: clip
        style: bg:on-primary r:20 shadow:1/2
        - node: div
          box:
            abs: [22,198 362x51]
            rel: [0,0 362x51]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:12/14
          style: border-b:1px seed-indigo@14
          - node: span
            box:
              abs: [36,210 26x26]
              rel: [14,12 26x26]
            style: bg:surface-container-high r:8 op:0.5
          - node: div
            box:
              abs: [78,210 260x26]
              rel: [56,12 260x26]
            - node: span
              box:
                abs: [78,210 90x11]
                rel: [0,0 90x11]
              style: bg:surface-container-high r:6 op:0.5
            - node: span
              box:
                abs: [78,227 60x9]
                rel: [0,17 60x9]
              style: bg:surface-container-high r:6 op:0.4
        - node: div
          box:
            abs: [22,249 362x51]
            rel: [0,51 362x51]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:12/14
          style: border-b:1px seed-indigo@14
          - node: span
            box:
              abs: [36,261 26x26]
              rel: [14,12 26x26]
            style: bg:surface-container-high r:8 op:0.5
          - node: div
            box:
              abs: [78,261 260x26]
              rel: [56,12 260x26]
            - node: span
              box:
                abs: [78,261 120x11]
                rel: [0,0 120x11]
              style: bg:surface-container-high r:6 op:0.5
            - node: span
              box:
                abs: [78,278 80x9]
                rel: [0,17 80x9]
              style: bg:surface-container-high r:6 op:0.4
        - node: div
          box:
            abs: [22,300 362x50]
            rel: [0,102 362x50]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:12/14
          - node: span
            box:
              abs: [36,312 26x26]
              rel: [14,12 26x26]
            style: bg:surface-container-high r:8 op:0.5
          - node: div
            box:
              abs: [78,312 260x26]
              rel: [56,12 260x26]
            - node: span
              box:
                abs: [78,312 150x11]
                rel: [0,0 150x11]
              style: bg:surface-container-high r:6 op:0.5
            - node: span
              box:
                abs: [78,329 100x9]
                rel: [0,17 100x9]
              style: bg:surface-container-high r:6 op:0.4
    - node: div
      box:
        abs: [22,364 362x171]
        rel: [14,218 362x171]
      spacing: margin:0/0/14/0
      - node: div
        box:
          abs: [22,364 362x19]
          rel: [0,0 362x19]
        layout: flex:row gap:6
        spacing: pad:2/4/8/4
        - node: span
          box:
            abs: [26,366 70x9]
            rel: [4,2 70x9]
          style: bg:surface-container-high r:6 op:0.4
      - node: card
        mx: MxCard
        box:
          abs: [22,383 362x152]
          rel: [0,19 362x152]
        position: clip
        style: bg:on-primary r:20 shadow:1/2
        - node: div
          box:
            abs: [22,383 362x51]
            rel: [0,0 362x51]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:12/14
          style: border-b:1px seed-indigo@14
          - node: span
            box:
              abs: [36,395 26x26]
              rel: [14,12 26x26]
            style: bg:surface-container-high r:8 op:0.5
          - node: div
            box:
              abs: [78,395 260x26]
              rel: [56,12 260x26]
            - node: span
              box:
                abs: [78,395 90x11]
                rel: [0,0 90x11]
              style: bg:surface-container-high r:6 op:0.5
            - node: span
              box:
                abs: [78,412 60x9]
                rel: [0,17 60x9]
              style: bg:surface-container-high r:6 op:0.4
        - node: div
          box:
            abs: [22,434 362x51]
            rel: [0,51 362x51]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:12/14
          style: border-b:1px seed-indigo@14
          - node: span
            box:
              abs: [36,446 26x26]
              rel: [14,12 26x26]
            style: bg:surface-container-high r:8 op:0.5
          - node: div
            box:
              abs: [78,446 260x26]
              rel: [56,12 260x26]
            - node: span
              box:
                abs: [78,446 120x11]
                rel: [0,0 120x11]
              style: bg:surface-container-high r:6 op:0.5
            - node: span
              box:
                abs: [78,463 80x9]
                rel: [0,17 80x9]
              style: bg:surface-container-high r:6 op:0.4
        - node: div
          box:
            abs: [22,485 362x50]
            rel: [0,102 362x50]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:12/14
          - node: span
            box:
              abs: [36,497 26x26]
              rel: [14,12 26x26]
            style: bg:surface-container-high r:8 op:0.5
          - node: div
            box:
              abs: [78,497 260x26]
              rel: [56,12 260x26]
            - node: span
              box:
                abs: [78,497 150x11]
                rel: [0,0 150x11]
              style: bg:surface-container-high r:6 op:0.5
            - node: span
              box:
                abs: [78,514 100x9]
                rel: [0,17 100x9]
              style: bg:surface-container-high r:6 op:0.4
```

## State: Results (full — differs too much from base)

```text
- node: app
  mx: MxScaffold
  box:
    abs: [8,8 390x780]
    rel: [8,8 390x780]
  layout: flex:col
  position: pos:relative clip
  style: bg:surface
  - node: statusbar
    box:
      abs: [8,8 390x44]
      rel: [0,0 390x44]
    layout: flex:row justify:between align:center
    spacing: pad:0/24
    - node: span
      text: 9:41
      box:
        abs: [32,21 28x18]
        rel: [24,13 28x18]
      style: font:14/600 color:font-headline
    - node: span
      box:
        abs: [314,24 60x12]
        rel: [306,16 60x12]
      layout: flex:row gap:4 align:center
      - node: svg
        box:
          abs: [314,24 16x12]
          rel: [0,0 16x12]
        position: clip
      - node: svg
        box:
          abs: [334,24 14x12]
          rel: [20,0 14x12]
        position: clip
      - node: svg
        box:
          abs: [352,24 22x12]
          rel: [38,0 22x12]
        position: clip
  - node: appbar
    mx: MxAppBar
    box:
      abs: [8,52 390x56]
      rel: [0,44 390x56]
    layout: flex:row gap:8 align:center
    spacing: pad:0/8
    - node: icon-btn
      mx: MxIconButton
      box:
        abs: [16,62 36x36]
        rel: [8,10 36x36]
      layout: flex:row justify:center align:center
      position: pos:relative
      style: r:999
      - node: span
        box:
          abs: [24,70 20x20]
          rel: [8,8 20x20]
        layout: flex:row
        - node: icon:arrow-left
          box:
            abs: [24,70 20x20]
            rel: [0,0 20x20]
          position: clip
    - node: div
      box:
        abs: [60,60 330x40]
        rel: [52,8 330x40]
      layout: flex:row gap:8 align:center
      flex: grow:1 basis:0 layout_hint:expanded
      spacing: pad:0/12
      style: bg:on-primary r:12 border:1px seed-indigo
      - node: span
        box:
          abs: [73,73 15x15]
          rel: [13,13 15x15]
        layout: flex:row
        - node: icon:search
          box:
            abs: [73,73 15x15]
            rel: [0,0 15x15]
          position: clip
      - node: span
        text: 연구
        box:
          abs: [96,70 249x20]
          rel: [36,10 249x20]
        layout: flex:row align:center
        flex: grow:1 basis:0 layout_hint:expanded
        position: clip
        style: font:14/600 color:font-headline
        - node: span
          box:
            abs: [124,72 2x16]
            rel: [28,2 2x16]
          spacing: margin:0/0/0/2
          style: bg:seed-indigo
      - node: icon-btn
        mx: MxIconButton
        box:
          abs: [353,68 24x24]
          rel: [293,8 24x24]
        layout: flex:row justify:center align:center
        position: pos:relative
        style: r:999
        - node: span
          box:
            abs: [355,70 20x20]
            rel: [2,2 20x20]
          layout: flex:row
          - node: icon:x-circle
            box:
              abs: [355,70 20x20]
              rel: [0,0 20x20]
            position: clip
  - node: scroll-x
    box:
      abs: [22,108 362x28]
      rel: [14,100 362x28]
    layout: flex:row gap:6
    repeat: x5(unit=1)
    position: layout_hint:scroll
    - node: button
      item: item[1]
      text: All
      mx: ?
      box:
        abs: [22,108 54x28]
        rel: [0,0 54x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:seed-indigo font:12/600 color:on-primary text:center r:999
      - node: span
        text: 18
        box:
          abs: [51,115 14x15]
          rel: [29,7 14x15]
        style: font:12/700 color:on-primary text:center op:0.75
    - node: button
      item: item[2]
      text: Folders
      mx: ?
      box:
        abs: [82,108 94x28]
        rel: [60,0 94x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [93,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:folder
          box:
            abs: [93,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 1
        box:
          abs: [157,115 7x15]
          rel: [76,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[3]
      text: Decks
      mx: ?
      box:
        abs: [181,108 87x28]
        rel: [159,0 87x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [192,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:layers
          box:
            abs: [192,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 2
        box:
          abs: [250,115 7x15]
          rel: [68,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[4]
      text: Flashcards
      mx: ?
      box:
        abs: [274,108 120x28]
        rel: [252,0 120x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [285,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:copy
          box:
            abs: [285,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 12
        box:
          abs: [369,115 14x15]
          rel: [95,7 14x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[5]
      text: Tags
      mx: ?
      box:
        abs: [400,108 77x28]
        rel: [378,0 77x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [411,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:tag
          box:
            abs: [411,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 3
        box:
          abs: [459,115 7x15]
          rel: [59,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
  - node: scroll
    box:
      abs: [8,146 390x642]
      rel: [0,138 390x642]
    flex: grow:1 basis:0 layout_hint:expanded
    repeat: x6(unit=1)
    spacing: pad:0/14/14/14
    position: layout_hint:scroll
    scroll: scrollh:755
    - node: ov
      text: 18 results for "연구"
      box:
        abs: [26,154 126x13]
        rel: [18,8 126x13]
      style: font:10.5/600 color:on-surface-variant tracking:0.6 op:0.85
    - node: div
      item: item[2]
      box:
        abs: [22,179 362x81]
        rel: [14,33 362x81]
      spacing: margin:0/0/16/0
      - node: div
        box:
          abs: [22,179 362x23]
          rel: [0,0 362x23]
        layout: flex:row justify:between align:center
        spacing: pad:0/4/8/4
        - node: div
          box:
            abs: [26,179 101x15]
            rel: [4,0 101x15]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [26,181 11x11]
              rel: [0,2 11x11]
            layout: flex:row
            - node: icon:folder
              box:
                abs: [26,181 11x11]
                rel: [0,0 11x11]
              position: clip
          - node: span
            text: Folders
            box:
              abs: [43,179 58x15]
              rel: [17,0 58x15]
            style: font:12/700 color:seed-indigo tracking:0.4
          - node: span
            text: 1
            box:
              abs: [107,179 19x15]
              rel: [81,0 19x15]
            spacing: pad:0/6
            style: bg:surface-container font:12/700 color:on-surface-variant r:999
      - node: card
        mx: MxCard
        box:
          abs: [22,202 362x58]
          rel: [0,23 362x58]
        position: clip
        style: bg:on-primary r:20 shadow:1/2
        - node: div
          box:
            abs: [22,202 362x58]
            rel: [0,0 362x58]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          - node: div
            box:
              abs: [36,218 26x26]
              rel: [14,16 26x26]
            layout: flex:row justify:center align:center
            style: bg:seed-indigo@12 r:8
            - node: span
              box:
                abs: [43,225 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:folder
                box:
                  abs: [43,225 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,213 265x36]
              rel: [56,11 265x36]
            - node: div
              text: Korean — TOPIK roots
              box:
                abs: [78,213 265x20]
                rel: [0,0 265x20]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
              - node: mark
                text: 연구
                box:
                  abs: [128,214 30x18]
                  rel: [50,1 30x18]
                spacing: pad:0/2
                style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.18) font:14/700 color:seed-indigo tracking:-0.1 r:3
            - node: div
              text: 4 subfolders · 286 cards
              box:
                abs: [78,234 265x15]
                rel: [0,21 265x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [355,224 15x15]
              rel: [333,22 15x15]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [355,224 15x15]
                rel: [0,0 15x15]
              layout: flex:row
              - node: icon:chevron-right
                box:
                  abs: [355,224 15x15]
                  rel: [0,0 15x15]
                position: clip
    - node: div
      item: item[3]
      box:
        abs: [22,276 362x140]
        rel: [14,130 362x140]
      spacing: margin:0/0/16/0
      - node: div
        box:
          abs: [22,276 362x23]
          rel: [0,0 362x23]
        layout: flex:row justify:between align:center
        spacing: pad:0/4/8/4
        - node: div
          box:
            abs: [26,276 85x15]
            rel: [4,0 85x15]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [26,278 11x11]
              rel: [0,2 11x11]
            layout: flex:row
            - node: icon:layers
              box:
                abs: [26,278 11x11]
                rel: [0,0 11x11]
              position: clip
          - node: span
            text: Decks
            box:
              abs: [43,276 43x15]
              rel: [17,0 43x15]
            style: font:12/700 color:accent tracking:0.4
          - node: span
            text: 2
            box:
              abs: [92,276 19x15]
              rel: [66,0 19x15]
            spacing: pad:0/6
            style: bg:surface-container font:12/700 color:on-surface-variant r:999
      - node: card
        mx: MxCard
        box:
          abs: [22,299 362x117]
          rel: [0,23 362x117]
        position: clip
        style: bg:on-primary r:20 shadow:1/2
        - node: div
          box:
            abs: [22,299 362x59]
            rel: [0,0 362x59]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          style: border-b:1px seed-indigo@14
          - node: div
            box:
              abs: [36,315 26x26]
              rel: [14,16 26x26]
            layout: flex:row justify:center align:center
            style: bg:accent@12 r:8
            - node: span
              box:
                abs: [43,322 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:layers
                box:
                  abs: [43,322 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,310 234x36]
              rel: [56,11 234x36]
            - node: div
              text: vocabulary
              box:
                abs: [78,310 234x20]
                rel: [0,0 234x20]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
              - node: mark
                text: 연구
                box:
                  abs: [78,311 30x18]
                  rel: [0,1 30x18]
                spacing: pad:0/2
                style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.18) font:14/700 color:seed-indigo tracking:-0.1 r:3
            - node: div
              text: 62 cards · Korean / TOPIK II
              box:
                abs: [78,331 234x15]
                rel: [0,21 234x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [324,319 46x18]
              rel: [302,20 46x18]
            layout: flex:row gap:6 align:center
            - node: span
              text: 8 due
              box:
                abs: [324,319 46x18]
                rel: [0,0 46x18]
              layout: flex:row align:center
              spacing: pad:0/7
              style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.1) font:12/700 color:seed-indigo r:999
        - node: div
          box:
            abs: [22,358 362x58]
            rel: [0,59 362x58]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          - node: div
            box:
              abs: [36,374 26x26]
              rel: [14,16 26x26]
            layout: flex:row justify:center align:center
            style: bg:accent@12 r:8
            - node: span
              box:
                abs: [43,381 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:layers
                box:
                  abs: [43,381 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,369 265x36]
              rel: [56,11 265x36]
            - node: div
              text: Academic terms
              box:
                abs: [78,369 265x20]
                rel: [0,0 265x20]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
              - node: mark
                text: 연구
                box:
                  abs: [148,370 30x18]
                  rel: [70,1 30x18]
                spacing: pad:0/2
                style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.18) font:14/700 color:seed-indigo tracking:-0.1 r:3
            - node: div
              text: 34 cards · Korean / Advanced
              box:
                abs: [78,390 265x15]
                rel: [0,21 265x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [355,380 15x15]
              rel: [333,22 15x15]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [355,380 15x15]
                rel: [0,0 15x15]
              layout: flex:row
              - node: icon:chevron-right
                box:
                  abs: [355,380 15x15]
                  rel: [0,0 15x15]
                position: clip
    - node: div
      item: item[4]
      box:
        abs: [22,432 362x199]
        rel: [14,286 362x199]
      spacing: margin:0/0/16/0
      - node: div
        box:
          abs: [22,432 362x23]
          rel: [0,0 362x23]
        layout: flex:row justify:between align:center
        spacing: pad:0/4/8/4
        - node: div
          box:
            abs: [26,432 134x15]
            rel: [4,0 134x15]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [26,434 11x11]
              rel: [0,2 11x11]
            layout: flex:row
            - node: icon:copy
              box:
                abs: [26,434 11x11]
                rel: [0,0 11x11]
              position: clip
          - node: span
            text: Flashcards
            box:
              abs: [43,432 85x15]
              rel: [17,0 85x15]
            style: font:12/700 color:mastery tracking:0.4
          - node: span
            text: 12
            box:
              abs: [134,432 26x15]
              rel: [108,0 26x15]
            spacing: pad:0/6
            style: bg:surface-container font:12/700 color:on-surface-variant r:999
        - node: button
          text: See all
          mx: ?
          box:
            abs: [329,432 51x15]
            rel: [307,0 51x15]
          layout: flex:row gap:3 align:center
          style: font:12/600 color:seed-indigo text:center
          - node: span
            box:
              abs: [369,434 11x11]
              rel: [40,2 11x11]
            layout: flex:row
            - node: icon:chevron-right
              box:
                abs: [369,434 11x11]
                rel: [0,0 11x11]
              position: clip
      - node: card
        mx: MxCard
        box:
          abs: [22,455 362x176]
          rel: [0,23 362x176]
        position: clip
        style: bg:on-primary r:20 shadow:1/2
        - node: div
          box:
            abs: [22,455 362x59]
            rel: [0,0 362x59]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          style: border-b:1px seed-indigo@14
          - node: div
            box:
              abs: [36,471 26x26]
              rel: [14,16 26x26]
            layout: flex:row justify:center align:center
            style: r:8
            - node: span
              box:
                abs: [43,478 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:copy
                box:
                  abs: [43,478 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,466 265x36]
              rel: [56,11 265x36]
            - node: div
              text: 자
              box:
                abs: [78,466 265x20]
                rel: [0,0 265x20]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
              - node: mark
                text: 연구
                box:
                  abs: [78,467 30x18]
                  rel: [0,1 30x18]
                spacing: pad:0/2
                style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.18) font:14/700 color:seed-indigo tracking:-0.1 r:3
            - node: div
              text: researcher / nhà nghiên cứu
              box:
                abs: [78,487 265x15]
                rel: [0,21 265x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [355,477 15x15]
              rel: [333,22 15x15]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [355,477 15x15]
                rel: [0,0 15x15]
              layout: flex:row
              - node: icon:chevron-right
                box:
                  abs: [355,477 15x15]
                  rel: [0,0 15x15]
                position: clip
        - node: div
          box:
            abs: [22,514 362x59]
            rel: [0,59 362x59]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          style: border-b:1px seed-indigo@14
          - node: div
            box:
              abs: [36,530 26x26]
              rel: [14,16 26x26]
            layout: flex:row justify:center align:center
            style: r:8
            - node: span
              box:
                abs: [43,537 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:copy
                box:
                  abs: [43,537 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,525 265x36]
              rel: [56,11 265x36]
            - node: div
              text: 하다
              box:
                abs: [78,525 265x20]
                rel: [0,0 265x20]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
              - node: mark
                text: 연구
                box:
                  abs: [78,526 30x18]
                  rel: [0,1 30x18]
                spacing: pad:0/2
                style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.18) font:14/700 color:seed-indigo tracking:-0.1 r:3
            - node: div
              text: to research, to study
              box:
                abs: [78,546 265x15]
                rel: [0,21 265x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [355,536 15x15]
              rel: [333,22 15x15]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [355,536 15x15]
                rel: [0,0 15x15]
              layout: flex:row
              - node: icon:chevron-right
                box:
                  abs: [355,536 15x15]
                  rel: [0,0 15x15]
                position: clip
        - node: div
          box:
            abs: [22,573 362x58]
            rel: [0,118 362x58]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          - node: div
            box:
              abs: [36,589 26x26]
              rel: [14,16 26x26]
            layout: flex:row justify:center align:center
            style: r:8
            - node: span
              box:
                abs: [43,596 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:copy
                box:
                  abs: [43,596 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,584 265x36]
              rel: [56,11 265x36]
            - node: div
              text: 실
              box:
                abs: [78,584 265x20]
                rel: [0,0 265x20]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
              - node: mark
                text: 연구
                box:
                  abs: [78,585 30x18]
                  rel: [0,1 30x18]
                spacing: pad:0/2
                style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.18) font:14/700 color:seed-indigo tracking:-0.1 r:3
            - node: div
              text: lab, research room
              box:
                abs: [78,605 265x15]
                rel: [0,21 265x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [355,595 15x15]
              rel: [333,22 15x15]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [355,595 15x15]
                rel: [0,0 15x15]
              layout: flex:row
              - node: icon:chevron-right
                box:
                  abs: [355,595 15x15]
                  rel: [0,0 15x15]
                position: clip
    - node: div
      item: item[5]
      box:
        abs: [22,647 362x193]
        rel: [14,501 362x193]
      spacing: margin:0/0/16/0
      - node: div
        box:
          abs: [22,647 362x23]
          rel: [0,0 362x23]
        layout: flex:row justify:between align:center
        spacing: pad:0/4/8/4
        - node: div
          box:
            abs: [26,647 75x15]
            rel: [4,0 75x15]
          layout: flex:row gap:6 align:center
          - node: span
            box:
              abs: [26,649 11x11]
              rel: [0,2 11x11]
            layout: flex:row
            - node: icon:tag
              box:
                abs: [26,649 11x11]
                rel: [0,0 11x11]
              position: clip
          - node: span
            text: Tags
            box:
              abs: [43,647 33x15]
              rel: [17,0 33x15]
            style: font:12/700 color:streak tracking:0.4
          - node: span
            text: 3
            box:
              abs: [82,647 19x15]
              rel: [56,0 19x15]
            spacing: pad:0/6
            style: bg:surface-container font:12/700 color:on-surface-variant r:999
      - node: card
        mx: MxCard
        box:
          abs: [22,670 362x170]
          rel: [0,23 362x170]
        position: clip
        style: bg:on-primary r:20 shadow:1/2
        - node: div
          box:
            abs: [22,670 362x57]
            rel: [0,0 362x57]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          style: border-b:1px seed-indigo@14
          - node: div
            box:
              abs: [36,685 26x26]
              rel: [14,15 26x26]
            layout: flex:row justify:center align:center
            style: r:8
            - node: span
              box:
                abs: [43,692 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:tag
                box:
                  abs: [43,692 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,681 266x34]
              rel: [56,11 266x34]
            - node: div
              text: research
              box:
                abs: [78,681 266x18]
                rel: [0,0 266x18]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
            - node: div
              text: 24 cards across all decks
              box:
                abs: [78,700 266x15]
                rel: [0,19 266x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [356,691 14x14]
              rel: [334,21 14x14]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [356,691 14x14]
                rel: [0,0 14x14]
              layout: flex:row
              - node: icon:arrow-up-right
                box:
                  abs: [356,691 14x14]
                  rel: [0,0 14x14]
                position: clip
        - node: div
          box:
            abs: [22,727 362x57]
            rel: [0,57 362x57]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          style: border-b:1px seed-indigo@14
          - node: div
            box:
              abs: [36,742 26x26]
              rel: [14,15 26x26]
            layout: flex:row justify:center align:center
            style: r:8
            - node: span
              box:
                abs: [43,749 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:tag
                box:
                  abs: [43,749 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,738 266x34]
              rel: [56,11 266x34]
            - node: div
              text: researcher
              box:
                abs: [78,738 266x18]
                rel: [0,0 266x18]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
            - node: div
              text: 8 cards across all decks
              box:
                abs: [78,757 266x15]
                rel: [0,19 266x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [356,748 14x14]
              rel: [334,21 14x14]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [356,748 14x14]
                rel: [0,0 14x14]
              layout: flex:row
              - node: icon:arrow-up-right
                box:
                  abs: [356,748 14x14]
                  rel: [0,0 14x14]
                position: clip
        - node: div
          box:
            abs: [22,784 362x56]
            rel: [0,114 362x56]
          layout: grid cols:3 gap:12 align:center
          spacing: pad:11/14
          - node: div
            box:
              abs: [36,799 26x26]
              rel: [14,15 26x26]
            layout: flex:row justify:center align:center
            style: r:8
            - node: span
              box:
                abs: [43,806 13x13]
                rel: [7,7 13x13]
              layout: flex:row
              - node: icon:tag
                box:
                  abs: [43,806 13x13]
                  rel: [0,0 13x13]
                position: clip
          - node: div
            box:
              abs: [78,795 266x34]
              rel: [56,11 266x34]
            - node: div
              text: research-lab
              box:
                abs: [78,795 266x18]
                rel: [0,0 266x18]
              position: clip
              style: font:14/600 color:font-headline tracking:-0.1
            - node: div
              text: 5 cards across all decks
              box:
                abs: [78,814 266x15]
                rel: [0,19 266x15]
              spacing: margin:1/0/0/0
              position: clip
              style: font:12/400 color:on-surface-variant
          - node: div
            box:
              abs: [356,805 14x14]
              rel: [334,21 14x14]
            layout: flex:row gap:6 align:center
            - node: span
              box:
                abs: [356,805 14x14]
                rel: [0,0 14x14]
              layout: flex:row
              - node: icon:arrow-up-right
                box:
                  abs: [356,805 14x14]
                  rel: [0,0 14x14]
                position: clip
    - node: div
      item: item[6]
      text: Showing top results · matches are case-insensitive
      box:
        abs: [22,856 362x31]
        rel: [14,710 362x31]
      spacing: pad:4/0/12/0
      style: font:12/400 color:on-surface-variant text:center
```

## State: No results (full — differs too much from base)

```text
- node: app
  mx: MxScaffold
  box:
    abs: [8,8 390x780]
    rel: [8,8 390x780]
  layout: flex:col
  position: pos:relative clip
  style: bg:surface
  - node: statusbar
    box:
      abs: [8,8 390x44]
      rel: [0,0 390x44]
    layout: flex:row justify:between align:center
    spacing: pad:0/24
    - node: span
      text: 9:41
      box:
        abs: [32,21 28x18]
        rel: [24,13 28x18]
      style: font:14/600 color:font-headline
    - node: span
      box:
        abs: [314,24 60x12]
        rel: [306,16 60x12]
      layout: flex:row gap:4 align:center
      - node: svg
        box:
          abs: [314,24 16x12]
          rel: [0,0 16x12]
        position: clip
      - node: svg
        box:
          abs: [334,24 14x12]
          rel: [20,0 14x12]
        position: clip
      - node: svg
        box:
          abs: [352,24 22x12]
          rel: [38,0 22x12]
        position: clip
  - node: appbar
    mx: MxAppBar
    box:
      abs: [8,52 390x56]
      rel: [0,44 390x56]
    layout: flex:row gap:8 align:center
    spacing: pad:0/8
    - node: icon-btn
      mx: MxIconButton
      box:
        abs: [16,62 36x36]
        rel: [8,10 36x36]
      layout: flex:row justify:center align:center
      position: pos:relative
      style: r:999
      - node: span
        box:
          abs: [24,70 20x20]
          rel: [8,8 20x20]
        layout: flex:row
        - node: icon:arrow-left
          box:
            abs: [24,70 20x20]
            rel: [0,0 20x20]
          position: clip
    - node: div
      box:
        abs: [60,60 330x40]
        rel: [52,8 330x40]
      layout: flex:row gap:8 align:center
      flex: grow:1 basis:0 layout_hint:expanded
      spacing: pad:0/12
      style: bg:on-primary r:12 border:1px seed-indigo
      - node: span
        box:
          abs: [73,73 15x15]
          rel: [13,13 15x15]
        layout: flex:row
        - node: icon:search
          box:
            abs: [73,73 15x15]
            rel: [0,0 15x15]
          position: clip
      - node: span
        text: phras
        box:
          abs: [96,71 249x18]
          rel: [36,11 249x18]
        layout: flex:row align:center
        flex: grow:1 basis:0 layout_hint:expanded
        position: clip
        style: font:14/600 color:font-headline
        - node: span
          box:
            abs: [136,72 2x16]
            rel: [40,1 2x16]
          spacing: margin:0/0/0/2
          style: bg:seed-indigo
      - node: icon-btn
        mx: MxIconButton
        box:
          abs: [353,68 24x24]
          rel: [293,8 24x24]
        layout: flex:row justify:center align:center
        position: pos:relative
        style: r:999
        - node: span
          box:
            abs: [355,70 20x20]
            rel: [2,2 20x20]
          layout: flex:row
          - node: icon:x-circle
            box:
              abs: [355,70 20x20]
              rel: [0,0 20x20]
            position: clip
  - node: scroll-x
    box:
      abs: [22,108 362x28]
      rel: [14,100 362x28]
    layout: flex:row gap:6
    repeat: x5(unit=1)
    position: layout_hint:scroll
    - node: button
      item: item[1]
      text: All
      mx: ?
      box:
        abs: [22,108 46x28]
        rel: [0,0 46x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:seed-indigo font:12/600 color:on-primary text:center r:999
      - node: span
        text: 0
        box:
          abs: [51,115 7x15]
          rel: [29,7 7x15]
        style: font:12/700 color:on-primary text:center op:0.75
    - node: button
      item: item[2]
      text: Folders
      mx: ?
      box:
        abs: [74,108 94x28]
        rel: [52,0 94x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [85,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:folder
          box:
            abs: [85,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 0
        box:
          abs: [150,115 7x15]
          rel: [76,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[3]
      text: Decks
      mx: ?
      box:
        abs: [174,108 87x28]
        rel: [152,0 87x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [185,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:layers
          box:
            abs: [185,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 0
        box:
          abs: [243,115 7x15]
          rel: [68,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[4]
      text: Flashcards
      mx: ?
      box:
        abs: [267,108 113x28]
        rel: [245,0 113x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [278,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:copy
          box:
            abs: [278,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 0
        box:
          abs: [362,115 7x15]
          rel: [95,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
    - node: button
      item: item[5]
      text: Tags
      mx: ?
      box:
        abs: [386,108 77x28]
        rel: [364,0 77x28]
      layout: flex:row gap:5 align:center
      flex: shrink:0
      spacing: pad:0/10
      style: bg:on-primary font:12/600 color:font-headline text:center r:999 border:1px seed-indigo@14
      - node: span
        box:
          abs: [397,117 11x11]
          rel: [11,9 11x11]
        layout: flex:row
        - node: icon:tag
          box:
            abs: [397,117 11x11]
            rel: [0,0 11x11]
          position: clip
      - node: span
        text: 0
        box:
          abs: [444,115 7x15]
          rel: [59,7 7x15]
        style: font:12/700 color:font-headline text:center op:0.6
  - node: card
    mx: MxCard
    box:
      abs: [22,154 362x274]
      rel: [14,146 362x274]
    repeat: x2(unit=2)
    spacing: pad:36/22 margin:8/0/0/0
    style: bg:on-primary r:20 shadow:1/2
    - node: div
      item: item[1]
      box:
        abs: [177,190 52x52]
        rel: [155,36 52x52]
      layout: flex:row justify:center align:center
      spacing: margin:0/0/14/0
      style: bg:surface-container r:14
      - node: span
        box:
          abs: [192,205 22x22]
          rel: [15,15 22x22]
        layout: flex:row
        - node: icon:search-x
          box:
            abs: [192,205 22x22]
            rel: [0,0 22x22]
          position: clip
    - node: div
      text: No matches for "phras"
      box:
        abs: [44,256 318x21]
        rel: [22,102 318x21]
      spacing: margin:0/0/6/0
      style: font:16/700 color:font-headline text:center
    - node: div
      item: item[2]
      text: Check the spelling, or try a shorter keyword. Search runs across folders, decks, cards, and tags.
      box:
        abs: [44,283 318x65]
        rel: [22,129 318x65]
      spacing: pad:0/8 margin:0/0/16/0
      style: font:14/400/22 color:on-surface-variant text:center
    - node: div
      box:
        abs: [44,364 318x28]
        rel: [22,210 318x28]
      layout: flex:row wrap gap:6 justify:center
      - node: button
        text: phrase
        mx: ?
        box:
          abs: [71,364 80x28]
          rel: [27,0 80x28]
        layout: flex:row gap:5 align:center
        spacing: pad:0/12
        style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.1) font:12/600 color:seed-indigo text:center r:999
        - node: span
          box:
            abs: [83,373 11x11]
            rel: [12,9 11x11]
          layout: flex:row
          - node: icon:search
            box:
              abs: [83,373 11x11]
              rel: [0,0 11x11]
            position: clip
      - node: button
        text: phrasal
        mx: ?
        box:
          abs: [157,364 82x28]
          rel: [113,0 82x28]
        layout: flex:row gap:5 align:center
        spacing: pad:0/12
        style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.1) font:12/600 color:seed-indigo text:center r:999
        - node: span
          box:
            abs: [169,373 11x11]
            rel: [12,9 11x11]
          layout: flex:row
          - node: icon:search
            box:
              abs: [169,373 11x11]
              rel: [0,0 11x11]
            position: clip
      - node: button
        text: phrasing
        mx: ?
        box:
          abs: [245,364 90x28]
          rel: [201,0 90x28]
        layout: flex:row gap:5 align:center
        spacing: pad:0/12
        style: bg:color(srgb 0.321569 0.396078 0.960784 / 0.1) font:12/600 color:seed-indigo text:center r:999
        - node: span
          box:
            abs: [257,373 11x11]
            rel: [12,9 11x11]
          layout: flex:row
          - node: icon:search
            box:
              abs: [257,373 11x11]
              rel: [0,0 11x11]
            position: clip
```

## State: Error (full — differs too much from base)

```text
- node: app
  mx: MxScaffold
  box:
    abs: [8,8 390x780]
    rel: [8,8 390x780]
  layout: flex:col
  position: pos:relative clip
  style: bg:surface
  - node: statusbar
    box:
      abs: [8,8 390x44]
      rel: [0,0 390x44]
    layout: flex:row justify:between align:center
    spacing: pad:0/24
    - node: span
      text: 9:41
      box:
        abs: [32,21 28x18]
        rel: [24,13 28x18]
      style: font:14/600 color:font-headline
    - node: span
      box:
        abs: [314,24 60x12]
        rel: [306,16 60x12]
      layout: flex:row gap:4 align:center
      - node: svg
        box:
          abs: [314,24 16x12]
          rel: [0,0 16x12]
        position: clip
      - node: svg
        box:
          abs: [334,24 14x12]
          rel: [20,0 14x12]
        position: clip
      - node: svg
        box:
          abs: [352,24 22x12]
          rel: [38,0 22x12]
        position: clip
  - node: appbar
    mx: MxAppBar
    box:
      abs: [8,52 390x56]
      rel: [0,44 390x56]
    layout: flex:row gap:8 align:center
    spacing: pad:0/8
    - node: icon-btn
      mx: MxIconButton
      box:
        abs: [16,62 36x36]
        rel: [8,10 36x36]
      layout: flex:row justify:center align:center
      position: pos:relative
      style: r:999
      - node: span
        box:
          abs: [24,70 20x20]
          rel: [8,8 20x20]
        layout: flex:row
        - node: icon:arrow-left
          box:
            abs: [24,70 20x20]
            rel: [0,0 20x20]
          position: clip
    - node: div
      box:
        abs: [60,60 330x40]
        rel: [52,8 330x40]
      layout: flex:row gap:8 align:center
      flex: grow:1 basis:0 layout_hint:expanded
      spacing: pad:0/12
      style: bg:on-primary r:12 border:1px seed-indigo
      - node: span
        box:
          abs: [73,73 15x15]
          rel: [13,13 15x15]
        layout: flex:row
        - node: icon:search
          box:
            abs: [73,73 15x15]
            rel: [0,0 15x15]
          position: clip
      - node: span
        text: 연구
        box:
          abs: [96,70 249x20]
          rel: [36,10 249x20]
        layout: flex:row align:center
        flex: grow:1 basis:0 layout_hint:expanded
        position: clip
        style: font:14/600 color:font-headline
        - node: span
          box:
            abs: [124,72 2x16]
            rel: [28,2 2x16]
          spacing: margin:0/0/0/2
          style: bg:seed-indigo
      - node: icon-btn
        mx: MxIconButton
        box:
          abs: [353,68 24x24]
          rel: [293,8 24x24]
        layout: flex:row justify:center align:center
        position: pos:relative
        style: r:999
        - node: span
          box:
            abs: [355,70 20x20]
            rel: [2,2 20x20]
          layout: flex:row
          - node: icon:x-circle
            box:
              abs: [355,70 20x20]
              rel: [0,0 20x20]
            position: clip
  - node: card
    mx: MxCard
    box:
      abs: [22,116 362x280]
      rel: [14,108 362x280]
    spacing: pad:40/22 margin:8/0/0/0
    style: bg:on-primary r:20 shadow:1/2
    - node: div
      box:
        abs: [177,156 52x52]
        rel: [155,40 52x52]
      layout: flex:row justify:center align:center
      spacing: margin:0/0/14/0
      style: bg:#dc4848@10 r:14
      - node: span
        box:
          abs: [192,171 22x22]
          rel: [15,15 22x22]
        layout: flex:row
        - node: icon:cloud-off
          box:
            abs: [192,171 22x22]
            rel: [0,0 22x22]
          position: clip
    - node: div
      text: Search didn't run
      box:
        abs: [44,222 318x21]
        rel: [22,106 318x21]
      spacing: margin:0/0/6/0
      style: font:16/700 color:font-headline text:center
    - node: div
      text: Your library is safe on this device. Try again in a moment.
      box:
        abs: [44,249 318x43]
        rel: [22,133 318x43]
      spacing: margin:0/0/16/0
      style: font:14/400/22 color:on-surface-variant text:center
    - node: pill-btn
      text: Retry
      mx: MxPrimaryButton
      box:
        abs: [157,308 93x48]
        rel: [135,192 93x48]
      layout: flex:row gap:6 justify:center align:center
      spacing: pad:0/18
      style: bg:seed-indigo font:14/600 color:on-primary text:center tracking:0.1 r:12
      - node: span
        box:
          abs: [175,325 14x14]
          rel: [18,17 14x14]
        layout: flex:row
        - node: icon:refresh-cw
          box:
            abs: [175,325 14x14]
            rel: [0,0 14x14]
          position: clip
```
