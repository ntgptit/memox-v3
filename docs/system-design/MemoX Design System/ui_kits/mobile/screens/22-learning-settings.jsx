...
(function () {
  ...
  const RowHead = ({ icon, tint, title, desc, on, disabled }) => (
  ...
  const GoalCard = ({ on }) => (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: S(4) }}>
      <RowHead icon="target" tint="var(--memox-status-new)" title="Daily goal" desc={on ? 'Cards to study each day' : 'Turned off — study freely'} on={on} />
      {on && (
        <React.Fragment>
  ...
  const ReminderCard = ({ on, disabled }) => (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: S(4), opacity: disabled ? 'var(--memox-op-disabled)' : 1 }}>
