...
| **Cách chạy** | **Inner loop khi đang dev**: `node tool/verify/run.mjs --quick [--test <paths>]` (analyze + test nhắm đích, nhanh, KHÔNG marker — không dùng để commit). **Cuối task**: `--test <paths>` (code) hoặc `--docs` (docs-only) hoặc auto-detect / `--code` / `--full`. Regen toàn bộ golden (vd sau khi đổi `test/flutter_test_config.dart`): `--full --update-goldens` (nhánh `--full` đã forward `--update-goldens` xuống `flutter test`). **CJK golden font:** goldens render Korean/kanji qua font fallback `Noto Sans KR` (theme `MxTypography.fontFamilyFallback`); repo chỉ giữ một **subset** (`assets/fonts/noto_sans_kr/NotoSansKR-goldensubset.ttf`, ~77 KB) chứa đúng các codepoint CJK xuất hiện trong golden text. Khi thêm golden có ký tự CJK mới (hiện tofu □), regen subset: `pip install fonttools` rồi `pyftsubset <NotoSansKR full>.ttf --unicodes=U+0020-007E,U+00A0-024F --text="<các ký tự CJK trong golden>" --output-file=assets/fonts/noto_sans_kr/NotoSansKR-goldensubset.ttf` (hoặc dùng `fontTools.subset` như commit gốc), rồi `--full --update-goldens`. Font KHÔNG bundle trong `pubspec.yaml` (device dùng CJK hệ thống). |
