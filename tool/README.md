...
| **Cách chạy** | `node tool/parity/report.mjs` (bảng; `--ssim`, `--check [--max <pct>] [--min-ssim V]`, `--screen`, `--json`) · `node tool/parity/token_lint.mjs [--check\|--json]` · `node tool/parity/node_audit.mjs` (per-node MISSING?/COLOR?/SHIFT? toàn app) · **closed loop**: `node tool/parity/structural_inventory.mjs --dump <json> --spec <md>` (node thiếu thật theo hình học; mặc định FIX, ngoại lệ có-docs qua `intent-ledger.json`; dump sinh bởi `test/support/structural_dump.dart`) · `node tool/parity/design_watch.mjs --check` (gate: design đổi ⇒ FE/golden/docs phải đổi theo, `--update` để re-baseline) · `node tool/parity/structural_audit.mjs` (đếm bug structural toàn app: 0 hiện tại, `--check` gate, `--bugs` liệt kê). Dump sinh khi golden test chạy (hook `dumpStructure`). Schema + vòng khép kín: `tool/parity/README.md`. |
