...
| **Cách chạy** | `node tool/parity/report.mjs` (bảng markdown; `--ssim`, `--check [--max <pct>] [--min-ssim V]`, `--screen <id>`, `--json`) · `node tool/parity/token_lint.mjs [--check\|--json]` · `node tool/parity/node_audit.mjs` (per-node MISSING?/COLOR?/SHIFT? cho toàn app; `--missing`, `--theme`, `--screen`, `--json`). Schema map + chi tiết: `tool/parity/README.md`. |
