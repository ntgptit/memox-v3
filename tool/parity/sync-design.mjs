...
// (v3) deterministically, WITHOUT the human being asked each time.
//
...
// is NOT CI-able (see docs/design/design-sync-process.md). So this script drives that
// CLI as a nested, plan-bounded `DesignSync` push: it computes the kit files changed in
// a git range, then has the CLI write the modified/added ones and delete the removed
// ones. The finalize_plan boundary makes it impossible to touch anything outside the
// computed paths.
//
// Usage:
...
// (this surface may not). Build a tight, plan-bounded prompt and drive that CLII.
