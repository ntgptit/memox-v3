...
class _FakeTtsService implements TtsService {
  ...
  @override
  Future<List<TtsVoice>> availableVoices(String lang) async => <TtsVoice>[];
...
class _FakeTtsSettingsRepository implements TtsSettingsRepository {
  ...
  Future<Result<TtsSettings>> load() async =>
      const Result<TtsSettings>.ok(TtsSettings.defaults);

  @override
  Future<Result<void>> save(TtsSettings s) async => const Result<void>.ok(null);
}
...
void main() {
  ...
  group('SpeakFlashcardUseCase', () {
    ...
    test(
      'T7: unsupported target language → no engine call (silent ok)',
      () async {
        final Result<void> result = await useCase.speakFlashcardFront(
