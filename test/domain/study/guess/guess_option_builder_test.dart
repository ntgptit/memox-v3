...
void main() {
  ...
  test('filters duplicate backs and falls back to the available decoys', () {
    ...
    final List<String> ids = GuessOptionBuilder.build(
      sessionId: 'session-2',
      current: current,
      scopeCards: scopeCards,
    ).map((option) => option.flashcard.id).toList(growable: false);
    ...
    expect(ids, contains(current.id));
    expect(ids, contains('card-2'));
  });
}
