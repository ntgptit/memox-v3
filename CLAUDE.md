...
- **KHÔNG để kit (mock design) drift khỏi Claude Design v3.** Bất kỳ thay đổi nào dưới `docs/system-design/MemoX Design System/ui_kits/**` (JSX screens, `_shared.jsx`, `index.html`, regenerated specs/shots) PHẢI được đồng bộ lên Claude Design project **v3** (`c83bf9df-9f34-4480-8351-3acbaa1e3246`). Đây là **standing authorization (PO 2026-06-24) — KHÔNG hỏi lại**. **Tự động hoá:** `.githooks/pre-push` tự chạy `node tool/parity/sync-design.mjs` mỗi khi `git push` có kit change (chỉ kích hoạt khi kit thật sự đổi); chạy tay khi cần: `node tool/parity/sync-design.mjs [<from-ref>]`. Push cần một `claude` CLI đã design-login (script drive nested `claude -p`); thiếu auth → script/hook báo lỗi **non-fatal** → ghi `design-sync: skipped (no design-authorized CLI)` thay vì drift im lặng. Tắt tạm: `MEMOX_NO_DESIGN_SYNC=1`. Chi tiết: `docs/design/design-sync-process.md`.
